...
void draw() {
  ...
  for (int y=0; y<img.height; y++) {
    for (int x=0; x<img.width; x++) {
      ...
      newImage.pixels[pos]=color(red(img.pixels[pos]));
  ...
  for (int y=0; y<img.height; y++) {
    for (int x=0; x<img.width; x++) {
      int pos=y*img.width+x;
      int novoValor = (int) red(newImage.pixels[pos]) +25;
...
for (int y=0; y<img.height; y++) {
    for (int x=0; x<img.width; x++) {
      int pos=y*img.width+x;
      if(red(newImage.pixels[pos])>150 && y>0 && y<637)newImage.pixels[pos]=color(0);
