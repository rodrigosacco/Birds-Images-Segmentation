...
//Filtro Brilho
  for(int y = 0; y < img.height; y++){
   for(int x = 0; x < img.width; x++){
    int pos = y*img.width + x;
    int novoValor = (int) red(newImg.pixels[pos]) - 0;
...
//Filtro de Limiarização
  for(int y = 0; y < img.height; y++){
   for(int x = 0; x < img.width; x++){
    int pos = y*img.width + x;
    if(red(newImg.pixels[pos]) < 209 && y > 60 && y < 526 && x > 190 && x < 425) newImg.pixels[pos] = color(img.pixels[pos]);
